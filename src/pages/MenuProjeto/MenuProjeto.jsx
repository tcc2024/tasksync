...
import ModalCadastroTarefa from '../../Componets/Modal/ModalCadastroTarefa';
...
export default function MenuProjeto() {
    ...
    async function AtualizarStatusTarefa(tarefa, status) {
        try {
            await ApiService.put(`/tarefa/AtualizarStatusTarefa/${tarefa}/${status - 1}`)
        } catch (error) {
            console.log(error);
        }
    }
    ...
    async function BuscarProjetos() {
        try {
            ...
            const listaDeProjetos = json.map(function (obj) {
                return {
                    value: obj.id,
                    label: obj.nome
                };
    ...
    async function BuscarTarefas() {
        try {
            ...
            const response = await ApiService.get(`/tarefa/projeto/${projetoSelecionado.value}/tarefas`)
            const json = response.data;

            const statusTodo = json.map(function (obj) {
                if (obj && obj.status === "0") {
                    return {
                        id: obj.id,
                        title: obj.nome
            ...
            const statusDoing = json.map(function (obj) {
                if (obj && obj.status === "1") {
                    return {
                        id: obj.id,
                        title: obj.nome
            ...
            const statusDone = json.map(function (obj) {
        ...
        <div className={styles.container}>
            ...
            <div className={styles.appContainer}>
                <div className={styles.header}>
                    <Select options={projetos} value={projetoSelecionado} className={styles.select} placeholder="Projetos..." onChange={(projeto) => setProjetoSelecionado(projeto)}></Select>
                    <button
